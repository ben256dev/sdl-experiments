...
bool load_pipeline_config(const std::string& path, PipelineConfig& out, Uint32 reflected_color_attachments) {
    std::ifstream f(path);
    if (!f.good()) {
        out.blends.assign(reflected_color_attachments ? reflected_color_attachments : 1, JsonBlend{});
        return false;
    }
    json j;
    f >> j;
    ...
    if (j.contains("vertex_layout")) out.vertex_layout_auto = (j["vertex_layout"].get<std::string>() == "auto");
